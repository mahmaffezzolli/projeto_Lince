...
class PricePurchaseDateScreen extends StatelessWidget {
  ...
  void _selectDate(BuildContext context, ValueSetter<String> onDateSelected) async {
  ...
  void _registerVehicle(BuildContext context) {
    try {
      final vehicleListProvider = Provider.of<VehicleListProvider>(context, listen: false);

      print('Price: ${priceController.text}');
      print('Purchase Date: ${purchaseDateController.text}');
      print('Model: ${modelController.text}');
      print('Brand: ${brandController.text}');
      print('Manufacture Year: ${manufactureYearController.text}');
      ...
      if (success == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Falha ao registrar o veículo. Verifique os campos obrigatórios.'),
          duration: Duration(seconds: 2),
        ));
      }
    } catch (e) {
